...
const TABS = ['Overview', 'Safety Controls', 'Audit Log']
...
export default function SupervisorPage() {
  ...
  const [auditLog, setAuditLog] = useState([
    { time: '09:14:18', operator: 'student_01', type: 'INFO', event: 'Session Start', detail: 'SES-0042 / Teleoperation' },
    { time: '09:14:20', operator: 'system', type: 'INFO', event: 'Readiness Check', detail: 'All checks passed' },
    { time: '09:14:25', operator: 'student_01', type: 'CMD', event: 'Motion Armed', detail: 'Remote controller enabled motion mode' },
    { time: '09:21:05', operator: 'system', type: 'WARN', event: 'Thermal Alert', detail: 'L_KNEE 58C (threshold 60C)' },
  ])

  useEffect(() => {
    robotAPI.getSafetyConfig().then(config => {
  ...
  const handleApplySafety = async () => {
    try {
      ...
      window.alert('Safety limits applied.')
    } catch (error) {
  ...
  const addAudit = (operator, type, event, detail) => {
    ...
    setAuditLog(previous => [{ time, operator, type, event, detail }, ...previous])
  }

  const exportCSV = () => {
    const rows = [['Time', 'Operator', 'Type', 'Event', 'Detail'], ...auditLog.map(entry => [entry.time, entry.operator, entry.type, entry.event, entry.detail])]
    const csv = rows.map(row => row.join(',')).join('\n')
    const link = document.createElement('a')
    link.href = `data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`
    link.download = 'chadwick_audit.csv'
    link.click()
  }
        ...
        {TABS.map((tab, index) => (
          <button
            ...
            style={{
              padding: '10px 20px',
              border: 'none',
              borderBottom: `2px solid ${activeTab === index ? 'var(--accent2)' : 'transparent'}`,
              background: 'transparent',
              color: activeTab === index ? 'var(--accent2)' : 'var(--dim)',
          ...
          <Panel style={{ overflowY: 'auto' }}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 10, color: 'var(--dim)' }}>Session: Teleoperation / SES-0042</div>
              </div>
            ...
            <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 10, color: 'var(--dim)', textAlign: 'center', marginBottom: 14 }}>
              Overrides all operators instantly
            </div>

            <SectionLabel>Current Limits</SectionLabel>
            {[
              ['MAX SPEED', `${safetyConfig.max_speed} m/s`],
              ['TURN RATE', `${safetyConfig.turn_rate} deg/s`],
              ['GEOFENCE', safetyConfig.active_zone],
              ['TEMP WARN', `${safetyConfig.temp_warn} C`],
            ].map(([key, value]) => (
              <div
                key={key}
                style={{
              ...
              >
                ...
                <span style={{ color: 'var(--accent2)' }}>{value}</span>
              </div>
            ))}
      ...
      {activeTab === 1 && (
        ...
        >
          <Panel title="Motion Limits" style={{ overflowY: 'auto' }}>
            <SafetyCard title="Speed & Movement">
              {[
                ...
                { label: 'Turn Rate', id: 'turn_rate', min: 0, max: 100, step: 5, unit: 'deg/s' },
              ].map(field => (
                <SliderRow
                  key={field.id}
                  label={field.label}
                  ...
                  max={field.max}
                  step={field.step}
                  onChange={value => setLocalConfig(current => ({ ...current, [field.id]: value }))}
                />
              ))}
            ...
            <SafetyCard title="Force / Torque">
              <SliderRow
                ...
                value={localConfig.max_torque_pct}
                unit="%"
                min={10}
                max={100}
                step={5}
                onChange={value => setLocalConfig(current => ({ ...current, max_torque_pct: value }))}
              />
              <ApplyBtn onClick={handleApplySafety} />
            </SafetyCard>
          </Panel>
          ...
          <Panel title="Geofence & Thresholds" style={{ overflowY: 'auto' }}>
            <SafetyCard title="Active Zone">
              {['LAB G12', 'CORRIDOR A', 'DEMO AREA B'].map(zone => (
                <div key={zone} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', fontSize: 12 }}>
                  <span>{zone}</span>
                  <Toggle
                    on={localConfig.active_zone === zone}
                    onChange={() => setLocalConfig(current => ({ ...current, active_zone: zone }))}
                  />
                </div>
              ))}
              <ApplyBtn onClick={handleApplySafety} />
            ...
            <SafetyCard title="Thermal Thresholds">
              <SliderRow
                label="Warning Temp"
                value={localConfig.temp_warn}
                unit="C"
                min={40}
                max={80}
                onChange={value => setLocalConfig(current => ({ ...current, temp_warn: value }))}
              />
              <SliderRow
                label="Stop Temp"
                value={localConfig.temp_stop}
                unit="C"
                min={50}
                max={90}
                onChange={value => setLocalConfig(current => ({ ...current, temp_stop: value }))}
              />
              <ApplyBtn onClick={handleApplySafety} />
            </SafetyCard>
          </Panel>
          ...
          <Panel title="Role Permissions" style={{ overflowY: 'auto' }}>
            <SafetyCard title="Operator Can...">
              {[
                ['Teleop (basic)', true, false],
                ['Scripted actions', true, false],
                ['Teleoperation', true, false],
            ...
            <SafetyCard title="Recording Policy">
              {[
                ['Auto-record sessions', true],
                ['Save lidar scans', true],
                ['Face detection (blocked)', false],
              ].map(([label, on]) => (
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', fontSize: 12 }}>
                  <span>{label}</span>
                  <Toggle on={on} disabled={String(label).includes('blocked')} />
                </div>
              ))}
              <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 9, color: 'var(--dim)', marginTop: 10, lineHeight: 1.6 }}>
                All data stored on USyd on-prem.
                <br />
                No external cloud by default.
              </div>
            </SafetyCard>
          </Panel>
        </div>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <Panel style={{ height: '100%', overflowY: 'auto' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Share Tech Mono, monospace', fontSize: 11 }}>
              <thead>
                <tr>
                  {['Timestamp', 'Operator', 'Type', 'Event', 'Details'].map(header => (
                    <th
                      key={header}
                      style={{
                        textAlign: 'left',
              ...
              <tbody>
                {auditLog.map((entry, index) => (
                  <tr
                    ...
                    onMouseEnter={event => {
                      event.currentTarget.style.background = 'rgba(10,245,160,0.02)'
                    }}
                    onMouseLeave={event => {
                      event.currentTarget.style.background = 'transparent'
                    }}
                  >
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{entry.time}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{entry.operator}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.03)', color: typeColor[entry.type] }}>{entry.type}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{entry.event}</td>
                    <td style={{ padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,0.03)', color: 'var(--dim)' }}>{entry.detail}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div
      style={{
        ...
        border: '1px solid rgba(10,245,160,0.15)',
        borderRadius: 8,
        padding: 14,
        marginBottom: 12,
      }}
    >
      <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, fontWeight: 600, color: 'var(--accent2)', marginBottom: 10, letterSpacing: 1 }}>
        {title}
      </div>
      {children}
    </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: 5,
        border: '1px solid var(--accent2)',
        background: 'rgba(10,245,160,0.08)',
        color: 'var(--accent2)',
        fontFamily: 'Rajdhani, sans-serif',
        fontSize: 13,
        fontWeight: 600,
